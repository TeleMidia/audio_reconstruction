...
def get_dct_image(image):
    ...
    chan = image.shape[2]
    dct_image = np.zeros((width,height, chan), np.float32)
...
def restore_image(dct_image):
    ...
    chan = dct_image.shape[2]
    
    image = np.zeros((width,height, chan), np.uint8)
...
def clear_image_dct_coefficients(dct_image, coef_begin, coef_end):
    ...
    height = dct_image.shape[1]
    chan = dct_image.shape[2]
    new_dct_image = np.zeros((width,height, chan), np.float32)
